 ...
 app.post('/continue',{preHandler:[(app as any).authenticate]},async(req,rep)=>{
  ...
  try{
   return await prisma.$transaction(async tx=>{
    ...
    if(session.status!=='CREATED' || session.streak<1) throw new Error('Session finished')
    const result=flip()
    const win=result===body.side
    if(!win){
     await tx.coinflipSession.update({where:{id:session.id},data:{status:'FINISHED',finishedAt:new Date(),winAmount:0n}})
   ...
   if(e.message==='Forbidden') return rep.code(403).send({error:'Forbidden'})
   if(e.message==='Session finished') return rep.code(400).send({error:'Session finished'})
   if(e.code==='P2025') return rep.code(404).send({error:'Session not found'})
   throw e
  }
 })
 app.post('/cashout',{preHandler:[(app as any).authenticate]},async(req,rep)=>{
  ...
  try{
   return await prisma.$transaction(async tx=>{
    const session=await tx.coinflipSession.findUniqueOrThrow({where:{id:body.sessionId}})
    if(session.userId!==user.id) throw new Error('Forbidden')
    if(session.status!=='CREATED') throw new Error('Session finished')
    if(session.streak<1) throw new Error('Flip at least once')
   ...
   if(e.message==='Forbidden') return rep.code(403).send({error:'Forbidden'})
   if(e.message==='Session finished') return rep.code(400).send({error:'Session finished'})
   if(e.message==='Flip at least once') return rep.code(400).send({error:'Flip at least once'})
   if(e.code==='P2025') return rep.code(404).send({error:'Session not found'})
   throw e
  }
 })
