...
function multiplierFor(streak) { return Math.round(Math.pow(PER_FLIP_MULTIPLIER, streak) * 100) / 100; }
function payoutFor(bet, streak) { if (streak <= 0) return { multiplier: 0, payout: 0 }; const mult = multiplierFor(streak); return { multiplier: mult, payout: Math.round(bet * mult) }; }
function flip() { return Math.random() < 0.5 ? 'heads' : 'tails'; }
export async function coinflipRoutes(app) {
    app.post('/start', { preHandler: [app.authenticate] }, async (req, rep) => {
        const user = await getAuthUser(req);
        const parsed = startSchema.safeParse(req.body);
        if (!parsed.success)
            return rep.code(400).send({ error: 'Bet amount must be from 10 to 5,000,000' });
    ...
    app.post('/continue', { preHandler: [app.authenticate] }, async (req, rep) => {
        const user = await getAuthUser(req);
