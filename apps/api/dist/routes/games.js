...
export async function gameRoutes(app) {
    ...
    app.post('/:gameCode/bet', { preHandler: [app.authenticate] }, async (req, rep) => { const u = await getAuthUser(req); const { gameCode } = req.params; const parsed = schema.safeParse(req.body); if (!parsed.success) return rep.code(400).send({ error: 'Bet amount must be from 1 to 1,000,000' }); const body = parsed.data; if (body.requestId) {
