...
export async function gameRoutes(app) {
    ...
    app.post('/:gameCode/bet', { preHandler: [app.authenticate] }, async (req, rep) => { const u = await getAuthUser(req); const { gameCode } = req.params; const body = schema.parse(req.body); if (body.requestId) {
