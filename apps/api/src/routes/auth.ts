...
export async function authRoutes(app:FastifyInstance){
 app.post('/telegram', async(req,rep)=>{ const {initData,startParam}=z.object({initData:z.string().min(1),startParam:z.string().optional()}).parse(req.body); if(!verifyTelegramInitData(initData,process.env.TELEGRAM_BOT_TOKEN!)) return rep.code(401).send({error:'Invalid Telegram initData'}); const tg=parseTelegramUser(initData); if(!tg)return rep.code(400).send({error:'Telegram user missing'}); const user=await prisma.user.upsert({where:{telegramId:BigInt(tg.id)},update:{username:tg.username,firstName:tg.first_name,lastName:tg.last_name,photoUrl:tg.photo_url},create:{telegramId:BigInt(tg.id),username:tg.username,firstName:tg.first_name,lastName:tg.last_name,photoUrl:tg.photo_url,balance:1000000n}}); const token=app.jwt.sign({userId:user.id,telegramId:user.telegramId.toString()});
  ...
  return {token,referral,user:{id:fresh.id,playerId:publicPlayerId(fresh),telegramId:fresh.telegramId.toString(),username:fresh.username,firstName:fresh.firstName,lastName:fresh.lastName,photoUrl:fresh.photoUrl,balance:Number(fresh.balance)}} })
