...
import { playLimbo } from '../games/limbo.js'
...
const HISTORY_CODES=['dice','coinflip','roulette','mines','blackjack','baccarat','limbo']
...
function playGame(code:string,p:any){ if(code==='coinflip')return playCoinflip(p); if(code==='dice')return playDice(p); if(code==='roulette')return playRoulette(p); if(code==='drunkard-gate')return playDrunkardGate(p); if(code==='baccarat')return playBaccarat(p); if(code==='limbo')return playLimbo(p); throw new Error('Unknown game') }
function mapGameCode(code:string){ if(code==='coinflip')return 'COINFLIP'; if(code==='dice')return 'DICE'; if(code==='roulette')return 'ROULETTE'; if(code==='drunkard-gate')return 'DRUNKARD_GATE'; if(code==='mines')return 'MINES'; if(code==='blackjack')return 'BLACKJACK'; if(code==='baccarat')return 'DRUNKARD_GATE'; if(code==='limbo')return 'LIMBO' as any; throw new Error('Unknown game') }
