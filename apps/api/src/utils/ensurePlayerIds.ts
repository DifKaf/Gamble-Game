...
// Выдаёт индивидуальный playerId всем игрокам и держит нумерацию в диапазоне 10001+ (5 цифр).
//
...
export async function ensurePlayerIds(log?: { info: (msg: string) => void; warn: (msg: string) => void }) {
	...
	try {
		...
		// 3. Пятизначные ID: 1 -> 10001. Один раз, только для маленьких значений.
		const shifted = await prisma.$executeRawUnsafe(
			'UPDATE "User" SET "playerId" = "playerId" + 10000 WHERE "playerId" < 10000'
		)

		// 4. Следующие регистрации продолжают нумерацию с максимума.
		await prisma.$executeRawUnsafe(
			'SELECT setval(pg_get_serial_sequence(\'"User"\', \'playerId\'), GREATEST((SELECT COALESCE(MAX("playerId"), 10000) FROM "User"), 10000))'
		)
		...
		info(
			`playerId ready: ${r ? Number(r.total) : 0} users, range ${r?.min ?? '-'}..${r?.max ?? '-'}` +
				(shifted ? `, shifted ${shifted} legacy ids into the 10001+ range` : '')
		)
	} catch (err: any) {
